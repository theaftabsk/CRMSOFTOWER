...
import { CheckCircle2, AlertCircle, Loader2, ArrowLeft, Video } from 'lucide-react';
...
function ZoomCallbackContent() {
  ...
  const [message, setMessage] = useState('Completing Zoom OAuth Handshake...');
  ...
  useEffect(() => {
    ...
    if (error) {
      setStatus('error');
      setMessage(`Zoom Authorization declined or failed: ${error}`);
      return;
    }
  ...
  const exchangeCode = async (code: string) => {
    try {
      setMessage('Exchanging authorization code for Zoom Access & Refresh tokens...');
      const res = await api.post('/integrations/zoom/exchange', { 
        code,
        redirect_uri: window.location.origin + '/integrations/zoom/callback',
      });

      if (res && (res.success || res.statusCode === 201 || res.data?.success)) {
        setStatus('success');
        const email = res.data?.account_email || res.account_email || 'Zoom Account';
        setAccountEmail(email);
        setMessage(`Successfully connected ${email}! Live Zoom meetings are now ready.`);
        ...
        setStatus('error');
        setMessage(res?.error?.message || res?.message || 'Zoom token exchange failed. Please verify marketplace credentials.');
      }
    } catch (err: any) {
      setStatus('error');
      ...
      <div className="w-full max-w-md bg-white border border-[#E5E5E5] rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.02)] p-8 text-center space-y-5">
        {status === 'loading' && (
          <>
            ...
            <h2 className="text-base font-bold text-[#111111]">Authenticating with Zoom</h2>
        ...
        {status === 'success' && (
          <>
            ...
            <h2 className="text-base font-bold text-[#111111]">Zoom Connected Successfully!</h2>
            <p className="text-xs text-[#666666] leading-relaxed">{message}</p>
            {accountEmail && (
              <div className="p-3 bg-[#FAFAFA] rounded-lg border border-[#E5E5E5] text-xs font-mono text-[#111111] font-semibold flex items-center justify-center space-x-2">
        ...
        {status === 'error' && (
          <>
            ...
            <h2 className="text-base font-bold text-[#111111]">Zoom Connection Failed</h2>
